...
class Review {
    ...
    var dictionary: [String: Any] {
        ...
        return ["title": title, "text": text, "rating": rating, "reviewUserID": reviewUserID, "reviewUserEmail": reviewUserID, "date": timeIntervalDate, "documentID": documentID]
    }
    ...
    func deleteData(spot: Spot, completion: @escaping (Bool) -> ()) {
        ...
        db.collection("spots").document(spot.documentID).collection("reviews").document(documentID).delete { (error) in
            if let error = error {
                print("error deleting review documentID \(self.documentID)")
                completion(false)
            } else {
