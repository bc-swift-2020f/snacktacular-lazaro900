    ...
    func saveIfNewUser(completion: @escaping (Bool) -> ()) {
        ...
        userRef.getDocument { (document, error) in
            guard error == nil else {
                print("Could not access document for user \(self.documentID)")
                return completion(false)
            }
            guard document?.exists == false else {
                print("Document ofr user \(self.documentID) already exists")
            ...
            db.collection("users").document(self.documentID).setData(dataToSave) { (error) in
                guard error == nil else {
                    print("Could not save data for \(self.documentID)")
                    return completion(false)
                }
                return completion(true)
